...
"""
    ...
    # Four printed digits, all read by the OCR, plus one operator beside them that no token read —
    # the real «d) 20/18 − 2/5» of Toán 5 tập một p22, whose «−» the OCR dropped. In area that sign
    # is a rounding error (3.75 % of the block's glyph ink on the real page, 3.2 % here); as a SHAPE
    # it is an unbroken run half a bar long. Area alone would restore the expression without its
    # operator, which is the round-3 defect «an expression served with the wrong operator».
    DROPPED_OPERATOR = """\
............................................................
....####....####....####....####............................
....####....####....####....####............................
...
"""
    ...
    def test_an_operator_no_token_read_fails_on_its_shape_not_its_area(self):
        m = InkMask.from_ascii(self.DROPPED_OPERATOR)
        boxes = [(c / 60, 1 / 13, (c + 4) / 60, 12 / 13) for c in (4, 12, 20, 28)]
        share, glyph, un = V.ink_accounted(m, (0.0, 0.0, 1.0, 1.0), boxes, [],
                                           text_height=1 / 13, bar_len=12 / 60)
        self.assertLess(share, V.MAX_INK_UNACCOUNTED,
                        'by area alone this would have passed — the run length is what catches it')
        self.assertEqual(un, 6)
        run = V.widest_unaccounted_run(m, (0.0, 0.0, 1.0, 1.0), boxes, [],
                                       text_height=1 / 13, bar_len=12 / 60)
        self.assertGreater(run, V.MAX_UNACCOUNTED_RUN)

...
def route_book(doc, subject):
    ...
    stats = Counter()
    fps = glob.glob(f'poc-out/units-k12/{book}.json')
    if not fps:
    ...
    if offset is None:
        stats['no_offset'] += 1
        return [], [], stats
    units = [u for u in json.load(open(fps[0]))['units'] if u['role'] != 'SECTION_TEXT']
    ...
    for no, us in sorted(by_lesson.items()):
        ...
        last_context = None  # nearest preceding expository unit (no directive)
        for u in us:
            t = clean(u['text'])
            if t is None:
                continue
            ...
            if not labels:
                if len(t) >= MIN_PASSAGE:
                    last_context = (t, printed)
                continue
            if 'WRITE_TEXT' in labels and len(t) <= 400:
                writings.append(dict(book=book, lesson=no, page=printed, prompt=t,
                                     pattern='WRITE_TEXT', subject=subject, source='pattern-router-v1'))
                stats['WRITE_TEXT'] += 1
                continue
            if 'SELECT_MCQ' in labels:
                ...
                if parsed and last_context:
                    ...
                    readings.append(dict(
                        book=book, lesson=no, page=last_context[1], passage=last_context[0],
                        questions=[dict(prompt=prompt, page=printed, options=opts)],
                        pattern='SELECT_MCQ', subject=subject, source='pattern-router-v1'))
                    stats['SELECT_MCQ'] += 1
                else:
                    stats['SELECT_MCQ_dropped'] += 1
                continue
            if 'READ_TEXT' in labels and u['role'] in ('READING', 'EXERCISE', 'ACTIVITY'):
                # Reading = this passage + the next directive unit as its question.
                if len(t) < MIN_PASSAGE:
                ...
                if not q or len(q) < MIN_PROMPT:
                    ...
                    continue
                readings.append(dict(
                    book=book, lesson=no, page=printed, passage=t[:4000],

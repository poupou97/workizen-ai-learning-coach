...
class _HocCungSamAppState extends State<HocCungSamApp> {
  ...
  ///
  /// Cùng MỘT cây widget mà `onOpenSubjects` vẫn đẩy — tách ra hàm để tab và
  /// CTA cũ dùng chung, KHÔNG nhân bản màn (§9 cấm duplicate screens).
  ///
  ...
  /// Tab SAM mở bài có workspace của ĐÚNG lớp đang học. Không có bài ⇒ `null`
  /// ⇒ màn SAM ẩn lối vào ấy thay vì mở một màn rỗng.
  VoidCallback? _openWorkspaceLessonFromSam(BuildContext context) {
    final doc = _workspaceLessonFor(_profile!);
  ...
  Widget _homeChild() => _loading
      ...
      : FutureBuilder<MissionData>(
          ...
          builder: (context, snap) {
            ...
            // ⭐ Lệnh 52 — Home trở thành GỐC của tab «Trang chủ». Màn
            // không bị viết lại; chỉ đổi chỗ nó đứng trong cây (§3).
            return AppShell(
              home: MissionCenterScreen(
                ...
                onOpenSubjects: () async {
                  ...
                  // (`_bookshelfRoot`), không dựng lại một bản thứ hai.
                  await Navigator.of(context).push(
                    MaterialPageRoute(
              ...
              sam: SamHubScreen(
                ...
                lessonLabel: _workspaceLessonFor(_profile!)?.title,
              ),

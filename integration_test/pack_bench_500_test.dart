/// WAL-84 MỐC 500MB — bench pack SIDE-LOADED (không bundle asset: 500MB vào
...
void main() {
  ...
  testWidgets('pack 500MB side-loaded bench', (tester) async {
    ...
    final candidates = [
      '${dir.path}/sam-synthetic-500mb.db',
      '/storage/emulated/0/Android/data/ai.workizen.learningcoach/files/'
          'sam-synthetic-500mb.db',
    ];
    final path = candidates.firstWhere(
        (p) => File(p).existsSync(),
        orElse: () => '');
    if (path.isEmpty) {
      markTestSkipped('side-load chưa đẩy pack 500MB — bỏ qua, nói thật');
      return;
    }
    bench('500mb_file_size', File(path).lengthSync() ~/ 1048576, 'MB');
    final sw = Stopwatch()..start();
    final db = sqlite3.open(path);
    bench('500mb_cold_open', sw.elapsedMicroseconds / 1000.0);
    final count = db.select('SELECT COUNT(*) c FROM unit').first['c'] as int;
    bench('500mb_unit_count', count, 'rows');
    ...
    fts.sort();
    bench('500mb_fts_p50', fts[fts.length ~/ 2]);
    bench('500mb_fts_p95', fts[(fts.length * 0.95).floor()]);
    ...
    bench('500mb_graph_scope_indexed_20x', scope);
    db.dispose();
    expect(count, greaterThan(900000));
  });
}

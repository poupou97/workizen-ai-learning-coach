...
```
              SAM_READY = 1          ← HÔM NAY (và là NGUYÊN MẪU, trust=prototype)
                    │
   GATE 1 ──────────┤  SGK ↔ SGV ghép được, CHỨNG MINH (không phải khớp số bài)
                    │
   GATE 2 ──────────┤  SGV cấp được bằng chứng sư phạm ĐÁNG TIN
                    │
   GATE 3 ──────────┤  Bằng chứng ấy → hợp đồng Pedagogy Runtime
                    │
              SAM 1 → 10            POC
                    │

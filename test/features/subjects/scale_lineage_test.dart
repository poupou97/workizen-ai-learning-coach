...
LearningContext _ctx({
  LearningIntent? intent = LearningIntent.prepare,
...
void _expectLineage(LearningEvent e, {String? version = _packVersion}) {
  ...
  expect(
    e.lessonNo,
    _lesson,
    reason: '⭐⭐ audit C7: thiếu bài ⇒ Learning Map mù',
  );
  expect(
    e.knowledgeVersion,
    version,
...
void main() {
  group('(a) từng surface — fixture trong bộ nhớ', () {
    testWidgets(
      'Reader: câu mở ⇒ participation mang sách + bài + packVersion',
      (t) async {
    ...
    testWidgets(
      'Reader: pack CHƯA khai provenance ⇒ rơi về hằng cũ, không null',
      (t) async {
        List<LearningEvent> out = const [];
        await t.pumpWidget(
          MaterialApp(
    ...
    testWidgets(
      'SourceReader: lập trường ⇒ participation mang lineage + version',
      (t) async {
        List<LearningEvent> out = const [];
        await t.pumpWidget(
          MaterialApp(
        ...
        await t.scrollUntilVisible(
          find.text(kConclusionStances[0]),
          120,
    ...
    testWidgets('MapReader: nhận context ⇒ lineage (bài từ DiaMap.lesson) + '
      ...
      await t.pump();
      await t.scrollUntilVisible(
        find.text('Con đã chỉ được trên bản đồ ✅'),
        150,
        scrollable: find.byType(Scrollable).first,
      );
    ...
    testWidgets('MapReader KHÔNG context, DiaMap không lesson (pack hôm nay) ⇒ '
        'sách có, bài null, KHÔNG hiện trên Learning Map — không đoán', (
      t,
    ) async {
      const map = DiaMap(
        subject: 'LS&ĐL',
      ...
      List<LearningEvent> out = const [];
      await t.pumpWidget(
        packHost(MapReaderScreen(map: map, onFinished: (e) => out = e)),
      );
      await t.pump();
      await t.scrollUntilVisible(
        find.text('Con đã chỉ được trên bản đồ ✅'),
    ...
    test(
      'Tutor: bài mở TỪ pack ⇒ sự kiện mang sách + bài; bài CHỤP ⇒ null',
      () {
        final c = toan5Bai6;
        final fp = FractionProblem.parse('3/4 + 2/5')!;
        final scope = TutorScope.forProblem(
          c.conceptId,
          c.classifyCase('3/4 + 2/5'),
          c.stage,
          c.catalogue,
        );
    ...
    testWidgets(
      'Assessment: context về ĐÚNG cuốn ⇒ lineage; cuốn khác ⇒ bài null',
      (t) async {
        const items = [
          CorpusExercise(
            expr: '1/2 - 1/5',
  ...
  group(
    '(b) Book Home → Reader → kho → Learning Map (fixture KHTN 6 Bài 17)',
    () {
      testWidgets(
        '⭐⭐ học xong Bài 17 ⇒ sự kiện trong kho mang lineage + packVersion; '
        'mở lại Book Home ⇒ badge «Đã học», không còn «Chưa học»',
        (t) async {
          ...
          expect(
            find.descendant(
              of: again,
              matching: find.textContaining('Đã học ·'),
            ),
            findsOneWidget,
          ...
          expect(
            find.descendant(of: other, matching: find.textContaining('Đã học')),
            findsNothing,
          );
        },
      );
  ...
  group('(c) FILE THẬT — pack lớp 6 trên máy', () {
    test('Bài 17 «Tách chất khỏi hỗn hợp» có trong mục lục KHTN 6', () {
      ...
      // ⭐⭐ WAL-223 S1 — NGHĨA VỤ DƯƠNG KHÔNG ĐƯỢC TRƯỢT BẰNG markTestSkipped.
      //
      // Cổng này tồn tại để chứng minh pack mang lai lịch build. Trước đây,
      // ĐÚNG điều kiện mà nó phải đỏ — pack có mặt mà KHÔNG khai packVersion —
      // lại là một `markTestSkipped`. Thứ còn sót lại chỉ là «nếu có
      // packVersion thì nó không rỗng», một câu không ràng buộc ai cả.
      //
      // Tách hai nghĩa vụ khác nhau:
      //   pack KHÔNG có trên máy  ⇒ bỏ qua chính đáng (corpus không vào repo)
      //   pack CÓ mà thiếu lai lịch ⇒ ĐỎ, và phải kiểm TRƯỚC mọi return sớm
      expect(
        idx.packVersion,
        isNotNull,
        reason:
            '⭐ pack lớp 6 CÓ trên máy này nhưng không khai buildProvenance. '
            'Một pack không truy được về builder/git/flags nào là pack không '
            'kiểm được — vắng lai lịch KHÔNG phải là đạt.',
      );
      expect(idx.packVersion, isNotEmpty);

      // Nghĩa vụ RIÊNG, và có thể chưa tới lượt: hoạt động cho bài mẫu. Bỏ qua
      // ở đây không còn kéo theo việc bỏ qua lai lịch phía trên.
      final acts = idx.activitiesFor(book: _book, lessonNo: _lesson);
      if (acts.isEmpty) {
        markTestSkipped(
          'Bài 17 chưa có hoạt động trên pack này (chờ lane Python '
